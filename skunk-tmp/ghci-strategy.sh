...
#
#   Stage 2: Build most of the target-platform object files (.o, .obj).
...
echo "$(wc -l < ghci-objects.txt) target-platform objects to compile in stages 1 and 2"
...
then
	echo "$steps build steps in stage 1, including $objs_tgt target-platform objects"
else
	echo "$steps build steps in stage 1, inc. $objs_all objects ($objs_tgt for target platform)"
fi

# Sanity check
if [ $steps -lt 5000 -o $steps -gt 30000 -o $objs_tgt -gt 8000 ]
then
...
# Note: The size of this list does not match the $objs_tgt count that we
...
echo "$(wc -l < ghci-objects-stage2.tmp.txt) target-platform objects to compile in stage 2"
